...
var UserDefineArea = function (layerName) {
    ...
    _proto.start = (map) => {
        ...
        drawTool.on("drawend", function (param) {
          const { coordinate } = param
            ...
            // if (drawTool.options.mode === "Point") {
            //     const circle = new maptalks.Circle(
            //         [param.coordinate.x, param.coordinate.y],
            //         _this.pointCircleRadius,
            //         {
            //             symbol: symbol["Polygon"],
            //         }
            //     )

            //     layerCollection.push(circle);
            // }
    ...
    _proto.setRadius = (val) => {
        if (!val || !Number(val)) return;
        _this.pointCircleRadius = val;
    }
    _proto.endDraw = () => {
      _this.drawTool.endDraw();
  }
    ...
    _proto.addPanel = (params) => {
      const {layer, drawLayer, height, top, name, coordinate } = params
      const options = {
        altitude: height || 0,
        draggable: true,
        single: false,
      ...
      text-align: center;white-space: nowrap;">
        ...
        const marker_panel_draw = new maptalks.ui.UIMarker(coordinate, options)
        marker_panel.addTo(layer).show();
        // marker_panel.addTo(currDrawLayer).show();
        marker_panel_draw.addTo(drawLayer).show();
        return [marker_panel, marker_panel_draw]
    }
    _proto.exitDraw = () => {
        if (document.querySelector(".maptalks-canvas-layer")) {
            document.querySelector(".maptalks-canvas-layer").style.cursor = "auto"
        }
        // document.querySelector(".maptalks-canvas-layer") && document.querySelector(".maptalks-canvas-layer").style.cursor = "auto";
        if(_this.drawTool) {
    ...
    _proto.createLayer = (mode, geojson, radius = _this.pointCircleRadius) => {
